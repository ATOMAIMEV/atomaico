...
class Planet {
    ...
    update() {
        ...
        const y = Math.sin(this.angle) * (this.data.orbit * 0.5); // Elliptical factor matched to orbits

        // Z-index and scaling for 3D feel
...
function animate() {
    ...
    // Draw nebulas first
    particles.filter(p => p.type === 'nebula').forEach(p => {
        p.update();
        p.draw();
    });
    ...
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            if (entry.target.classList.contains('features-sync')) {
                // Animate all children of features-sync together
                Array.from(entry.target.children).forEach((child, index) => {
                    setTimeout(() => {
                        child.style.opacity = '1';
                        child.style.transform = 'translateY(0)';
                    }, index * 100); // 100ms staggered entry
                });
            } else {
